...
export function AiAdvisorCard({ pageContext, className, title = "AI Advisor" }: Props) {
  ...
  return (
    <Card className={cn("bg-white border border-border/60 shadow-sm", className)}>
      <CardContent className="p-4">
        ...
        {data?.context && !isLoading && (
          <div className="mt-3 pt-3 border-t border-border/50 grid grid-cols-2 gap-x-3 gap-y-1">
            <div>
        ...
        <Button
          variant="outline"
          size="sm"
          className="w-full h-7 text-[11px] gap-1.5 mt-3"
          onClick={() => refetch()}
          disabled={isFetching}
        >
